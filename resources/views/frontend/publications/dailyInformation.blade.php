...
@section('content')
  @include('frontend.partials.newsFlow')
  		<div class="container" style="text-align: center">
	  		<h2><u>@lang('home.daily information')</u></h2>
			...
			<table class="table table-striped table-bordered ">
			  ...
			  <tbody>
			    ...
			    <tr>
			    <td>{{$publication->date_nep}}</td>
			      <td>{{$publication->title_nep}}</td>
			      <td><a href="{{route('dailyInformationdownload',$publication->id)}}"><i class="fa fa-download" style="font-size:38px"></i></a></td>
			    </tr>
			    @endforeach
			    @elseif(App::isLocale('en'))
			    @foreach($publications as $publication)
			    <tr>
			    	<td>{{$publication->date_en}}</td>
